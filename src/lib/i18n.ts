...
const resources = {
  en: { translation: {
    ...
    game: {
      ...
      confirmSelfDestruct: 'Self-destruct this unit? It will be removed before upkeep. Worker cargo drops on this cell; there is no production refund.', healRequiresOwnCore: 'A Unit can only heal while sharing a cell with its own stationary Core.', healCoreMoving: 'Healing is unavailable because the Core is moving this Tick.',
  ...
  zh: { translation: {
    ...
    game: {
      ...
      confirmSelfDestruct: '确定自毁这个单位吗？它会在扣维护费前消失；Worker 携带的资源会掉在当前格，但不会返还生产费用。', healRequiresOwnCore: '单位只有与己方静止 Core 同格时才能恢复 HP。', healCoreMoving: 'Core 在本 Tick 发生迁移，无法恢复 HP。',
