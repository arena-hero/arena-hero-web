...
export function OAuthPage({ provider }: { provider: OAuthProvider }) {
  ...
  const complete = async () => { if (!signupToken) return; setBusy(true); try { const session = await api.completeOAuthSignup(provider, signupToken, username); setCSRF(session.csrf_token); navigate('/arena') } catch (cause) { setError(getErrorMessage(cause)) } finally { setBusy(false) } }
