...
export function RegisterPage() {
  ...
  return <AuthCard eyebrow="NEW SIGNAL" title={t('auth.create')} subtitle="Username: 3–24 lowercase letters, numbers or underscores.">
    {sent ? <div><div className="rounded-gold-lg border border-green-resource/20 bg-green-resource/5 p-5 text-sm leading-6 text-green-resource">{t('auth.verificationSent')}</div><button onClick={() => void api.resendVerification(values.email)} className="secondary-button mt-3 w-full">{t('auth.resend')}</button></div> : <form onSubmit={(event) => void submit(event)} className="space-y-4">
      ...
      <FormField label={t('auth.username')} name="username" autoComplete="username" required pattern="[a-z0-9_]{3,24}" value={values.username} onChange={(event) => update('username', event.target.value.toLowerCase())} />
